...
class ImprovedTrainer:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        # 编译模型以加速（需要PyTorch 2.0+）/ Compile model for acceleration (requires PyTorch 2.0+)
    ...
    def train_epoch(self, epoch):
        ...
        accumulation_steps = self.config_manager.get('training.gradient_accumulation_steps', 1)
        
        pbar = tqdm(self.train_loader, desc=f'Epoch {epoch}')
        self.optimizer.zero_grad() # 在循环开始前清零
        ...
        for batch_idx, (lyso, hpge) in enumerate(pbar):
            ...
            if self.use_amp:
                with autocast():
                    ...
                    loss = loss / accumulation_steps # 规范化损失
            ...
            if (batch_idx + 1) % accumulation_steps == 0:
                ...
                self.optimizer.zero_grad() # 仅在更新后清空梯度
            
            # loss.item() 返回的是整个批次的损失，乘以累积步数以反映真实损失
            ...
            if batch_idx % self.log_interval == 0:
                pbar_info = {'loss': loss.item() * accumulation_steps}
    ...
    def run_final_evaluation(self):
        ...
        # 我们使用验证集作为最终评估的测试集
        data_config = self.config_manager.get_data_config()
        test_dir = data_config.get('val_path')
        
        if not test_dir or not Path(test_dir).exists():
            print(f"Test directory '{test_dir}' not found. Skipping evaluation.")
            return

        output_dir = Path(self.config_manager.get('logging.log_dir', 'logs')) / 'final_evaluation'
        ...
        try:
            ...
            print("Error: evaluate_model.py not found. Make sure it is in the project root directory.")
